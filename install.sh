...
# Extract panel base URL from subscription URL
panel_base_url() {
  local url="$1"
  # Remove path after domain: https://panel.example.com:20576/sub/abc → https://panel.example.com:20576
  echo "$url" | awk -F/ '{print $1"//"$3}'
}

# Query panel for client info using subscription URL
# Works with any panel — fetches /client/{sub_id} and extracts visible text
panel_query() {
  local sub_url="$1"
  local sub_id panel_url response

  sub_id=$(sub_extract_id "$sub_url")
  panel_url=$(panel_base_url "$sub_url")

  [[ -n "$sub_id" ]] || return 1

  # Try common panel paths: /client/{id}, /sub/{id}, /api/user/{id}
  local paths=("/client/${sub_id}" "/sub/${sub_id}" "/api/user/${sub_id}")
  response=""
  for path in "${paths[@]}"; do
    response=$(curl -fsSL --max-time 10 --noproxy '*' "${panel_url}${path}" 2>/dev/null) && break
    response=""
  done

  [[ -n "$response" ]] || return 1

  # Generic extraction — no hardcoding of panel types
...
# 2. Generic HTML text extraction — get all visible label:value pairs
# Strip tags, normalize whitespace, extract meaningful lines
text = re.sub(r'<script[^>]*>.*?</script>', '', raw, flags=re.DOTALL)
text = re.sub(r'<style[^>]*>.*?</style>', '', text, flags=re.DOTALL)
text = re.sub(r'<[^>]+>', '\n', text)
text = re.sub(r'\n\s*\n', '\n', text).strip()

# Extract lines that look like "Label: Value" or "Label Value"
...
# Print as-is — the user sees what the panel shows
if unique:
    for l in unique[:30]:  # cap at 30 lines
        print(l)
    sys.exit(0)
...
SUBEOF
  ...
  case "${action}" in
    show)
      ...
      # Query panel API (auto-detects /client, /sub, /api/user paths)
      if [[ -f "$(sub_url_file)" ]]; then
        local sub_url_for_panel panel_info
        sub_url_for_panel=$(cat "$(sub_url_file)")
        panel_info=$(panel_query "$sub_url_for_panel" 2>/dev/null || echo "")
        if [[ -n "$panel_info" ]]; then
          section "Panel info"
          # Check if JSON (structured) or plain text (generic)
          if echo "$panel_info" | python3 -c "import sys,json; json.loads(sys.stdin.read())" 2>/dev/null; then
            echo "$panel_info" | python3 -c "
import sys, json
...
def exp(ts):
    ...
    try: return datetime.fromtimestamp(int(ts)).strftime('%d.%m.%Y, %H:%M:%S')
    except: return str(ts)
print(f'  ID:       {data.get(\"id\",\"N/A\")}')
print(f'  Status:   {data.get(\"status\",\"\") or (\"Active\" if data.get(\"enable\",True) else \"Disabled\")}')
print(f'  Uploaded: {fmt(data.get(\"up\",0))}')
print(f'  Downloaded: {fmt(data.get(\"down\",0))}')
print(f'  Limit:    {data.get(\"limit\",\"\") or \"Unlimited\"}')
if data.get('last_online'): print(f'  Last seen: {data[\"last_online\"]}')
print(f'  Expires:  {exp(data.get(\"expire\",\"\"))}')
" 2>/dev/null || echo "$panel_info"
          else
            # Plain text — show as-is (works for any panel format)
            echo "$panel_info" | sed 's/^/  /'
          fi
        fi
      fi
      ;;
